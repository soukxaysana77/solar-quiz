...
import { DEFAULT_MODE, modeOf } from "./modes.js";
...
export function getPlayers(room) {
    // คะแนนรายข้อแนบไปเฉพาะโหมดต่อตึก โหมดอื่นไม่ใช้ ไม่ต้องยิงข้อมูลเกินไปทุก broadcast
    const withGains = Boolean(modeOf(room).tower);
    return [...room.players.values()]
        .map(player => ({
            ...(withGains ? { gains: [...(player.gains ?? [])].map(gain => gain ?? 0) } : {}),
            id: player.id,
            name: player.name,
