...
import {
    BLACKHOLE_FAST_RATIO, BLACKHOLE_MAX_LEAD, BLACKHOLE_START_LEAD, BOSS_HEARTS, BOSS_HP_RATIO, END_WHEN_ALL_ANSWERED,
...
import {
    broadcastHosts, countActivePlayers, getPlayers, loadRoom, reopenRoom, roomMedia, rooms, send, sendLobby
} from "./rooms.js";
import {
    ...
    wss.on("connection", (ws, req) => {
        ...
        const onMessage = raw => {
            ...
            if (message.type === "HOST_ROOM") {
                ...
                // ห้องที่จบไปแล้วค้างอยู่ในหน่วยความจำ ถ้าไม่ล้าง host จะเจอหน้าแท่นรับรางวัลรอบเก่าแทนล็อบบี้
                if (room.phase === "final") reopenRoom(room);
                roomPin = room.pin;
                isHost = true;
                room.hosts.add(ws);
                ...
                if (room.phase === "question") {
                    ...
                    send(ws, "FINAL", { players: getPlayers(room), mode: room.mode, boss: room.boss ?? null });
                }
                return;
            }
            ...
            if (message.type === "JOIN_ROOM") {
                ...
                // พก playerId มาแต่ห้องไม่รู้จัก = แท็บค้างจากรอบเก่า (เซิร์ฟเวอร์รีสตาร์ต / หลุดนานเกิน / ห้องถูกเปิดใหม่)
                // ห้ามสร้างผู้เล่นใหม่ให้เงียบ ๆ ไม่งั้นแท็บที่ไม่มีใครดูอยู่จะโผล่เป็นผู้เล่นผีในล็อบบี้รอบใหม่
                // ให้หน้าเว็บพากลับไปกดเข้าห้องเองที่หน้า lobby แทน
                if (message.playerId && !claimed) {
                    send(ws, "ERROR", { code: "sessionExpired", message: "Your previous session ended. Please join again." });
                    return;
                }
            ...
            if (message.type === "START_GAME" && isHost && room.phase === "lobby") {
                ...
                // ตั้งเลือดบอสตามจำนวนคนตอนกดเริ่ม (คนเข้ามาทีหลังช่วยตีได้ แต่ไม่ทำให้บอสอึดขึ้น)
                const bossHp = Math.max(1, Math.round(room.players.size * room.slides.length * BOSS_HP_RATIO));
                room.boss = MODES[room.mode].boss
                    ? { hp: bossHp, maxHp: bossHp, hearts: BOSS_HEARTS, maxHearts: BOSS_HEARTS, lastDamage: 0, counter: false, result: null }
                    : null;
                sendQuestion(room);
                return;
            }
            ...
            if (message.type === "ANSWER" && room.phase === "question" && player) {
                ...
                const correct = slide.correctIndexes.includes(answer);
                // ข้อ "ไม่คิดคะแนน" ไม่นับ streak ด้วย ตอบถูกไม่ต่อ ตอบผิดก็ไม่ตัด
                if (slide.points > 0) {
                ...
                const rawBonus = correct && !mode.noStreak ? streakBonus(player.streak) : 0;
                // ตัวคูณคะแนนของข้อนี้ (สองเท่า / ไม่คิดคะแนน) คูณทั้งฐานและโบนัส
                base = Math.round(base * slide.points);
                const bonus = Math.round(rawBonus * slide.points);
