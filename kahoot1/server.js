...
const MODES = {
    ...
    blackhole: {
     ...
     blockchoice: {
        ...
        label: { th: "โดดลงหลุม", en: "Black Hole Run" },
        desc: {
            th: "ตอบถูกโดดลงblockที่ถืกต้อง ตอบผิดหลุมดำ",
            en: "Correct answers buy distance, wrong ones let the black hole close in"
        },
        color: "#0d8020",
        icon: "🕳️",
...
// ล้างสถานะการเล่นทั้งหมดให้ห้องกลับไปเป็นล็อบบี้ ผู้เล่นที่ยังอยู่ในห้องไม่ต้องเข้าใหม่
function resetRoom(room) {
    ...
    for (const player of room.players.values()) {
        ...
        player.survived = 0;
        player.lead = BLACKHOLE_START_LEAD;
    }
...
wss.on("connection", (ws, req) => {
    ...
    const onMessage = raw => {
        ...
        // START GAME (host)
        // เล่นชุดเดิมอีกรอบ: พาห้องกลับไปสถานะรอผู้เล่น ผลของรอบก่อนถูกบันทึกไปแล้วจึงไม่หาย
        if (message.type === "RESET_ROOM" && isHost && room.phase === "final") {
            resetRoom(room);
            sendLobby(room);
            return;
        }
