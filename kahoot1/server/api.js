...
import {
    ...
    deleteSession, externalizeMedia, insertQuiz, insertUser, persist, quizForEdit, quizOwner, selectQuiz, updateQuiz,
...
import { closeRoom, refreshRoom, rooms } from "./rooms.js";

// แก้ชุดคำถามได้เฉพาะเจ้าของ แอดมินแก้ได้ทุกชุด
const canEditQuiz = (user, row) => user.role === "admin" || row.owner_id === user.id;
...
// โฟลเดอร์ที่เบราว์เซอร์โหลดได้ นอกนั้นห้ามเสิร์ฟเด็ดขาด
// รากโปรเจกต์มีฐานข้อมูล (มี session token ที่ใช้สวมรอยได้) ซอร์สเซิร์ฟเวอร์ และ .git อยู่ด้วย
// ชื่อเทียบแบบตัวพิมพ์ตรงตัว /make/ บน Windows จึงไม่หลุดผ่านด่านล็อกอินของ /Make/
const PUBLIC_DIRS = new Set(["auth", "dashboard", "page", "Make", "host", "game", "lobby", "shared", "uploads"]);

// รูปที่ครูอัปโหลดอาจเป็น SVG ที่ฝังสคริปต์มา ถ้าเปิดไฟล์ตรง ๆ ต้องไม่ให้รันอะไรได้
// (ตอนแสดงผ่าน <img> เบราว์เซอร์ไม่รันสคริปต์ใน SVG อยู่แล้ว header นี้กันกรณีเปิดลิงก์ตรง)
...
async function handlePost(req, res, pathname) {
    ...
    if (pathname === "/api/quizzes") {
        ...
        try {
            ...
            // ส่ง pin มาด้วย = บันทึกทับชุดเดิม ไม่สร้าง PIN ใหม่ ผลการเล่นเก่าที่ผูกกับ PIN นี้จึงยังอยู่ครบ
            if (data.pin) {
                const pin = String(data.pin).trim();
                const row = quizForEdit.get(pin);
                if (!row) return sendJson(res, 404, { code: "notFound" });
                if (!canEditQuiz(user, row)) return sendJson(res, 403, { code: "forbidden" });
                updateQuiz.run(title, JSON.stringify(slides), pin);
                refreshRoom(pin);
                return sendJson(res, 200, { pin, title });
            }
...
async function handleGet(req, res, url) {
    ...
    // หน้า Make เปิดชุดเดิมมาแก้ ต้องเป็นเจ้าของหรือแอดมิน
    if (url.pathname === "/api/quizzes/get") {
        const user = currentUser(req);
        if (!user) return sendJson(res, 401, { code: "notLoggedIn" });
        const row = quizForEdit.get(String(url.searchParams.get("pin") || "").trim());
        if (!row) return sendJson(res, 404, { code: "notFound" });
        if (!canEditQuiz(user, row)) return sendJson(res, 403, { code: "forbidden" });
        let slides;
        try { slides = JSON.parse(row.slides_json); } catch { slides = []; }
        return sendJson(res, 200, { pin: row.pin, title: row.title, slides });
    }
    ...
    // % ที่เข้ารหัสผิดรูปทำให้ decodeURIComponent โยน error ถ้าไม่ดักไว้เซิร์ฟเวอร์ล่มทั้งเครื่อง
    let raw;
    try {
    ...
    // ตรวจจากพาธที่ resolve แล้ว ไม่ใช่สตริงดิบ เพราะ \ ใน URL กลายเป็นตัวคั่นโฟลเดอร์บน Windows ได้
    const parts = relative(ROOT, target).split(sep);
    if (!PUBLIC_DIRS.has(parts[0]) || parts.some(part => part.startsWith("."))) {
        res.writeHead(404);
    ...
    try {
        ...
        if (parts[0] === "uploads") Object.assign(headers, UPLOAD_HEADERS);
...
// handler เป็น async ถ้าโยน error ออกมาจะกลายเป็น unhandled rejection ซึ่งปิดโปรเซสทิ้งทั้งตัว
// ดักไว้ชั้นนอกสุด คำขอเดียวพังต้องไม่ลากทุกห้องที่กำลังเล่นอยู่ล่มตามไปด้วย
function failRequest(res, error) {
    console.error("request failed:", error);
    if (res.headersSent) return res.destroy();
    sendJson(res, 500, { code: "serverError" });
}
