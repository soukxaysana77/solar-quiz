document.addEventListener('DOMContentLoaded', () => {
    ...
    KG.i18n.apply();
    KG.i18n.mountToggle();
    // ป้ายบอกว่าใครล็อกอินอยู่ พร้อมปุ่มเข้า/ออกจากระบบ
    KG.auth.mountChip();
    ...
    pinForm.addEventListener('submit', (e) => {
        ...
        fetch('../api/join', {
            ...
            // เซิร์ฟเวอร์ส่งรหัสมา หน้าเว็บเป็นคนแปล ข้อความจะได้ตามภาษาที่เลือกอยู่
            if (!response.ok) throw new Error(result.code || 'joinFailed');
            sessionStorage.setItem('mangosgo.activePin', pinValue);
            window.location.href = `../lobby/?pin=${encodeURIComponent(pinValue)}`;
        }).catch((error) => {
            setLoadingState(false);
            const code = error.message;
            showInputError(() => (code === 'joinFailed' ? t('page.joinFailed') : t('error.' + code)));
        });
    });

    // Display inline visual error indicator
    // รับ "วิธีวาดข้อความ" ไม่ใช่ข้อความสำเร็จรูป จะได้วาดใหม่ได้ตอนสลับภาษา
    let redrawMessage = null;
    KG.i18n.onChange(() => { if (redrawMessage) formMessage.textContent = redrawMessage(); });

    function showInputError(draw) {
        pinInput.style.border = '2px solid #e21b3c';
        pinInput.style.animation = 'shake 0.3s ease-in-out';
        redrawMessage = draw;
        formMessage.textContent = draw();
