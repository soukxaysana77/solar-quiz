document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ประกาศไว้ตรงนี้เพราะตัวฟังด้านล่างอ่านค่านี้ได้ตั้งแต่ก่อนโค้ดส่วนล่างจะทำงาน
    let lastPlayers = [];

    // ต้องลงทะเบียนตรงนี้ ก่อนโค้ดที่ return ออกไปตอนไม่มี PIN
    // ไม่งั้นหน้าที่จบด้วย return จะไม่มีตัวฟังการสลับภาษาเลย
    KG.i18n.onChange(() => {
        if (lastPlayers.length) renderPlayers(lastPlayers);
        ...
        socket.onmessage = (event) => {
            ...
            if (msg.type === 'JOINED') {
                ...
                KG.preload(msg.media, (done, total) => {
                    if (!total) { setText(el.preload, null); return; }
                    setText(el.preload, () => (done < total
            ...
            if (msg.type === 'ROOM_STATE') {
                ...
                else if (msg.phase === 'final') setText(el.status, () => t('common.gameOver'));
                return;
            }
            ...
            else if (msg.type === 'FINAL') setText(el.status, () => t('common.gameOver'));
        };
    };
