...
- If saving to the server fails (e.g. the connection drops) the work is kept as a local draft so nothing is lost
...
Clicking the **Black Hole Run / Block Jump / Boss Raid** cards on the host screen opens a popup that loops a
...
mechanic) · `blocks` (answer-platform stage) · `boss` (Boss Raid) · `endWhenAlive` (how many survivors ends the game)
...
3. **Reopening a finished room starts a new round straight away.** Hitting "▶ Host a game" (or opening
   `/host/?pin=` again) puts the room back in the lobby. Players whose game page is still open are taken back to
   the waiting screen automatically with their score reset to 0 (no need to rejoin); players who left are removed.
