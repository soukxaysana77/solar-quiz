...
KG.sound = (() => {
    ...
    function noise({ at = 0, dur = 0.2, gain = 0.15, out = null }) {
        if (!unlocked || !ensure()) return;
        const start = ctx.currentTime + at;
    ...
    // เพลงวนสองแบบ: lobby สบาย ๆ ระหว่างรอคนเข้า / game จังหวะเร็วขึ้นมีกลองตอนกำลังเล่นให้ห้องมีพลัง
    // ทั้งสองเล่นผ่าน musicGain ที่เบากว่าเสียงเอฟเฟกต์ เสียงติ๊กนับถอยหลังกับเสียงเฉลยจึงยังได้ยินชัด
    const TRACKS = {
        lobby: {
            every: 260, perBar: 4, leadGain: 0.09,
            bass: [110, 110, 146.83, 130.81],
            lead: [440, 523.25, 659.25, 523.25, 587.33, 523.25, 440, 392]
        },
        game: {
            every: 170, perBar: 8, leadGain: 0.06, beat: true,
            bass: [98, 98, 116.54, 87.31],
            lead: [392, 466.16, 587.33, 466.16, 392, 523.25, 622.25, 523.25,
                349.23, 440, 523.25, 440, 392, 466.16, 587.33, 698.46]
        }
    };
    let track = TRACKS.lobby;

    function musicLoop() {
        const bar = Math.floor(musicStep / track.perBar) % track.bass.length;
        // เพลงเร็วเล่นเบสแค่จังหวะคู่ ไม่งั้นเบสทุกจังหวะจะหนาจนกลบเสียงอื่น
        if (!track.beat || musicStep % 2 === 0) {
            tone({ freq: track.bass[bar], dur: 0.2, type: 'triangle', gain: 0.25, out: musicGain });
        }
        tone({ freq: track.lead[musicStep % track.lead.length], at: 0.02, dur: 0.14, type: 'square', gain: track.leadGain, out: musicGain });
        if (track.beat && musicStep % 4 === 0) noise({ dur: 0.06, gain: 0.05, out: musicGain });
        musicStep++;
    }
    ...
    function beginMusic() {
        ...
        musicStep = 0;
        musicLoop();
        musicTimer = setInterval(musicLoop, track.every);
    }
    ...
    return {
        ...
        // เรียกซ้ำด้วยเพลงเดิมไม่ทำอะไร (ROOM_STATE มาบ่อย) เรียกด้วยเพลงอื่นจะสลับเพลงทันที
        startMusic(name = 'lobby') {
            const next = TRACKS[name] || TRACKS.lobby;
            wantMusic = true;
            if (musicTimer && next === track) return;
