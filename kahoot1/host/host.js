document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderBlocksReveal = (players, options, correctIndex) => {
    ...
    const correctSlot = slotOf.get(correctIndex);

    // แท่นที่ไม่ใช่คำตอบถูกพังทิ้งทั้งหมด ไม่ว่าจะมีคนยืนอยู่หรือเปล่า เหลือแท่นคำตอบถูกไว้แท่นเดียว
    platforms.forEach((platform, position) => {
      if (position !== correctSlot) platform.classList.add('is-broken');
    });
    ...
    groups.forEach((group, slot) => {
      ...
      const mode = slot === correctSlot ? 'safe' : 'fallen';
  ...
  const handle = (msg) => {
    ...
    if (msg.type === 'ROOM_STATE') {
      ...
      if (msg.phase === 'lobby') {
        ...
        document.body.classList.remove('is-playing', 'is-question', 'is-reveal', 'is-final', 'is-run', 'is-blocks');
        if (cameFromFinal) {
          el.podiumView.hidden = true;
          el.playView.hidden = false;
          el.runView.hidden = true;
          el.blocksView.hidden = true;
          el.home.hidden = true;
          setMessage(null);
          setAnswered(null);
          el.podium.innerHTML = '';
    ...
    if (msg.type === 'LEADERBOARD') {
      ...
      if (msg.mode === 'blocks') {
        ...
        latestPlayers = msg.players;
        renderBlocksReveal(msg.players, currentOptions, msg.correctIndex);
        const correct = currentOptions.find((option) => option.index === msg.correctIndex);
        el.blocksAnswer.innerHTML = '';
        el.blocksAnswer.append(
          document.createTextNode(t('host.answer') + ' '),
          Object.assign(document.createElement('span'), { className: 'blocks-correct', textContent: correct?.text ?? '-' })
        );
        const out = msg.players.filter((player) => player.answerIndex !== msg.correctIndex).length;
    ...
    if (msg.type === 'FINAL') {
      stopTimer();
      stopCountdown();
      document.body.classList.remove('is-playing', 'is-question', 'is-reveal', 'is-run', 'is-blocks');
