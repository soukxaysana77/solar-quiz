...
// เปิดชุดเดิมกลับมาแก้ในหน้า Make และบันทึกทับ PIN เดิม (ข้อมูลอยู่ที่เซิร์ฟเวอร์ ล็อกอินเครื่องไหนก็เห็น)
export const quizForEdit = db.prepare("SELECT pin, title, slides_json, owner_id FROM quizzes WHERE pin = ?");
export const updateQuiz = db.prepare("UPDATE quizzes SET title = ?, slides_json = ? WHERE pin = ?");
