...
### Question sets can be replayed now

A PIN used to be good for one game only. Now the host screen shows a **🔁 Play this set again**
button once a game ends. It puts the room back into the lobby — players still connected do not have
to rejoin — and **the previous results are kept**, because every run is its own row in
`game_sessions`. One question set can be used across several classes and the results compared.
...
| Change permission rules / guard another page | `GUARDED_PAGES` in `server.js` |
| Change the sign-in page | `auth/` |
| Change the dashboards / reports | `dashboard/` and `dashboardFor()` in `server.js` |
