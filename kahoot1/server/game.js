...
export function finishQuestion(room) {
    ...
    // จดคะแนนรายข้อของทุกคน (ไม่ตอบ = 0) โหมดต่อตึกใช้ประกอบตึกใหม่จากศูนย์ได้ทุกเมื่อ
    // เช่น host รีเฟรชกลางเกม จอไม่ต้องเคยเห็นข้อก่อน ๆ มาก่อนก็วาดตึกถูก
    for (const player of room.players.values()) {
        (player.gains ??= [])[room.questionIndex] = room.answers.get(player.id)?.gain ?? 0;
    }
    ...
    // โหมดบอสรวมพลัง: รวมดาเมจทั้งห้อง ตอบไวตีแรงกว่า ตอบถูกไม่ถึงครึ่งห้องบอสสวนกลับ
    if (room.boss && !room.boss.result) {
        const boss = room.boss;
        let damage = 0;
        let correctCount = 0;
        for (const record of room.answers.values()) {
            if (!record.correct) continue;
            correctCount++;
            damage += record.fast ? 2 : 1;
        }
        boss.hp = Math.max(0, boss.hp - damage);
        boss.lastDamage = damage;
        boss.counter = boss.hp > 0 && correctCount < room.players.size / 2;
        if (boss.counter) boss.hearts = Math.max(0, boss.hearts - 1);
        if (boss.hp === 0) boss.result = "win";
        else if (boss.hearts === 0) boss.result = "lose";
    }
    ...
    broadcast(room, "LEADERBOARD", {
        ...
        boss: room.boss ?? null,
        isLastQuestion: room.questionIndex >= room.slides.length - 1 || isGameOver(room),
...
// จบก่อนหมดข้อได้สองทาง: ตกรอบจนเหลือคนตามเกณฑ์ หรือบอสรวมพลังรู้ผลแล้ว (บอสตาย/หัวใจหมด)
function isGameOver(room) {
    return isSurvivalOver(room) || Boolean(room.boss?.result);
}
...
function nextQuestion(room) {
    ...
    if (room.questionIndex >= room.slides.length - 1 || isGameOver(room)) {
        room.phase = "final";
        // หมดข้อแล้วบอสยังไม่ตาย = ทั้งห้องแพ้
        if (room.boss && !room.boss.result) room.boss.result = "lose";
        persist(updateProgress, "finished", room.questionIndex, room.questionStartedAt / 1000, room.pin);
        closeSession(room);
        broadcast(room, "FINAL", { players: getPlayers(room), mode: room.mode, boss: room.boss ?? null });
        return;
    }
    room.questionIndex++;
    sendQuestion(room);
...
export function sendLeaderboardTo(ws, room) {
    ...
    send(ws, "LEADERBOARD", {
        ...
        // host ที่ต่อกลับกลางช่วงเฉลยต้องรู้โหมดด้วย ไม่งั้นจอไม่รู้ว่าต้องวาดฉากของโหมดไหน
        mode: room.mode,
        alive: countAlive(room),
        boss: room.boss ?? null,
        isLastQuestion: room.questionIndex >= room.slides.length - 1 || isGameOver(room),
