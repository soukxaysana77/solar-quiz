...
KG.i18n = (() => {
    ...
    const DICT = {
        th: {
            ...
            'error.roomClosed': 'ชุดคำถามนี้ถูกลบแล้ว ห้องปิดแล้ว',
            'error.sessionExpired': 'การเข้าห้องครั้งก่อนหมดอายุแล้ว กดเข้าห้องอีกครั้งนะครับ',
            ...
            'host.scanToJoin': 'สแกนเพื่อเข้าเล่น',
            ...
            'make.loadFailed': 'เปิดชุดคำถามนี้ไม่ได้ (ไม่มีชุดนี้ หรือไม่ใช่ชุดของคุณ)',
            'make.loadError': 'โหลดชุดคำถามจากเซิร์ฟเวอร์ไม่สำเร็จ (เซิร์ฟเวอร์อาจปิดอยู่)\nข้อมูลบนเซิร์ฟเวอร์ยังอยู่ครบ ปุ่มบันทึกถูกล็อกไว้ไม่ให้หน้าเปล่าไปทับ ลองรีเฟรชหน้าอีกครั้ง',
            'make.loadErrorDraft': 'โหลดจากเซิร์ฟเวอร์ไม่สำเร็จ จึงเปิดร่างล่าสุดที่เก็บไว้ในเครื่องแทน\nกดบันทึกเมื่อเซิร์ฟเวอร์กลับมาได้เลย',
            'make.useDraft': 'พบร่างของชุดนี้ในเครื่องที่ยังบันทึกขึ้นเซิร์ฟเวอร์ไม่สำเร็จ และไม่ตรงกับที่บันทึกไว้\n\nOK = ใช้ร่างในเครื่อง\nยกเลิก = ใช้ข้อมูลบนเซิร์ฟเวอร์ (ลบร่างทิ้ง)',
            'make.home': 'หน้าแรก',
            ...
            'make.pickedCorrectMany': 'คำตอบที่ถูก: ข้อ {list}',
            'make.removeMedia': '✕ ลบรูป',
            'make.true': 'จริง',
            'make.false': 'เท็จ',
            ...
            'make.selMulti': 'มีคำตอบถูกได้หลายข้อ',
            ...
            'mode.boss': 'บอสรวมพลัง',
            'mode.tower': 'ต่อตึก',
            'demo.ok': 'เข้าใจแล้ว ✓',
            'demo.close': 'ปิด',
            'demo.holeCorrect': 'ตอบถูก → หนีห่างหลุมดำ',
            'demo.holeWrong': 'ตอบผิด → หลุมดำคืบเข้ามา',
            'demo.holeGone': 'ระยะหมด → โดนดูด',
            'demo.blocksJump': 'เลือกคำตอบ → โดดขึ้นแท่น',
            'demo.blocksBreak': 'แท่นผิดแตก → ตกหลุม',
            'demo.blocksAgain': 'ข้อถัดไป → กลับมาเล่นต่อ',
            'demo.bossHit': 'ตอบถูก → ตีบอส',
            'demo.bossFast': 'ตอบไว → ตีแรง x2',
            'demo.bossCounter': 'ถูกไม่ถึงครึ่ง → บอสสวนกลับ',
            'demo.bossTeam': 'ทั้งห้องชนะด้วยกัน',
            'demo.towerCorrect': 'ตอบถูก → ได้บล็อกต่อตึก',
            'demo.towerFast': 'ตอบไว → บล็อกสูงกว่า',
            'demo.towerWrong': 'ตอบผิด → บล็อกร่วง ตึกไม่โต',
            'demo.towerTop': 'ตึกสูงสุด = คะแนนสูงสุด',
            'host.towerTop': '🏗️ ตึกสูงสุดตอนนี้: {name} ({score} คะแนน)',
            'host.towerNone': 'ยังไม่มีใครได้บล็อก ข้อหน้าลุยใหม่!',
            'host.bossDamage': 'ทีมตีบอส {damage} ดาเมจ · บอสเหลือ {hp}',
            'host.bossCounter': 'ตีบอส {damage} ดาเมจ แต่ตอบถูกไม่ถึงครึ่ง บอสสวนกลับ! เหลือหัวใจ {hearts}',
            'host.bossWin': '🎉 ทั้งห้องชนะบอสแล้ว!',
            'host.bossLose': '💀 บอสชนะ... ลองใหม่อีกครั้ง',
            'game.bossHp': 'บอสเหลือ {hp}/{max}',
            'game.bossWin': '🎉 ทั้งห้องชนะบอส!',
            'game.bossLose': '💀 บอสชนะรอบนี้',
            'mode.rush': 'เร่งรีบ',
        ...
        en: {
            ...
            'error.roomClosed': 'This quiz was deleted, so the room is closed.',
            'error.sessionExpired': 'Your previous session ended. Please join again.',
            ...
            'make.loadFailed': 'Could not open this question set (it does not exist or is not yours)',
            'make.loadError': 'Could not load this question set from the server (it may be offline).\nNothing on the server was lost. Saving is locked so an empty page cannot overwrite it — try refreshing.',
            'make.loadErrorDraft': 'Could not reach the server, so your latest local draft was opened instead.\nSave again once the server is back.',
            'make.useDraft': 'Found a local draft of this set that never reached the server and differs from the saved version.\n\nOK = use the local draft\nCancel = use the server version (discard the draft)',
            'make.home': 'Home',
            'make.host': 'Host',
            ...
            'make.pickedCorrectMany': 'Correct answers: {list}',
            'make.removeMedia': '✕ Remove image',
            'make.true': 'True',
            'make.false': 'False',
            ...
            'make.selMulti': 'Multiple correct answers',
            ...
            'mode.boss': 'Boss Raid',
            'mode.tower': 'Tower Stack',
            'demo.ok': 'Got it ✓',
            'demo.close': 'Close',
            'demo.holeCorrect': 'Correct → escape the hole',
            'demo.holeWrong': 'Wrong → the hole closes in',
            'demo.holeGone': 'No distance left → sucked in',
            'demo.blocksJump': 'Pick an answer → jump on a block',
            'demo.blocksBreak': 'Wrong blocks break → fall',
            'demo.blocksAgain': 'Next question → back in',
            'demo.bossHit': 'Correct → hit the boss',
            'demo.bossFast': 'Fast → double damage',
            'demo.bossCounter': 'Under half right → boss hits back',
            'demo.bossTeam': 'The whole room wins together',
            'demo.towerCorrect': 'Correct → stack a block',
            'demo.towerFast': 'Fast → taller block',
            'demo.towerWrong': 'Wrong → the block falls off',
            'demo.towerTop': 'Tallest tower = top score',
            'host.towerTop': '🏗️ Tallest tower: {name} ({score} pts)',
            'host.towerNone': 'No blocks yet — go for the next one!',
            'host.bossDamage': 'The team dealt {damage} damage · boss has {hp} left',
            'host.bossCounter': '{damage} damage, but fewer than half got it right — the boss hits back! {hearts} hearts left',
            'host.bossWin': '🎉 The whole room beat the boss!',
            'host.bossLose': '💀 The boss wins... try again',
            'game.bossHp': 'Boss {hp}/{max}',
            'game.bossWin': '🎉 The room beat the boss!',
            'game.bossLose': '💀 The boss won this time',
            'mode.rush': 'Rush',
