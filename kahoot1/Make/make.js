document.addEventListener('DOMContentLoaded', () => {
    ...
    // true = เปิดชุดเดิมมาแก้แต่ยังไม่ได้ข้อมูลจริงจากเซิร์ฟเวอร์ ห้ามบันทึก ไม่งั้นหน้าเปล่าจะไปทับชุดคำถามของจริง
    ...
    // ร่างในเครื่องมีไว้กู้งานที่กดบันทึกไม่สำเร็จ (เช่นเซิร์ฟเวอร์ปิดอยู่ตอนกด) ใช้ได้เฉพาะชุดเดียวกับที่เปิดอยู่
    function readDraft() {
        try {
            const draft = JSON.parse(localStorage.getItem(STORAGE_KEY));
            if (!draft || !Array.isArray(draft.slides) || !draft.slides.length) return null;
            return (draft.pin || null) === currentPin ? draft : null;
        } catch (error) {
    ...
    const snapshot = (title, list) => JSON.stringify({ title: title || '', slides: normalizeSlides(list) });

    // 'ok' = ได้ข้อมูลแล้ว · 'missing' = ไม่มีชุดนี้/ไม่ใช่ของเรา · 'error' = ติดต่อเซิร์ฟเวอร์ไม่ได้ (ข้อมูลบนเซิร์ฟเวอร์ยังอยู่)
    async function loadFromServer(pin) {
        try {
            ...
            const result = await response.json();
            if (!response.ok || !Array.isArray(result.slides) || !result.slides.length) return 'error';
            ...
            console.warn('Unable to load quiz from server.', error);
            return 'error';
        }
    }
    ...
    titleInput.addEventListener('input', () => {
        if (!saveLocked && titleInput.value.trim().length > 0) {
    ...
    // ปุ่ม "+ Create" ถูกคอมเมนต์ออกจาก make.html ไปแล้ว ต้องเช็คก่อนผูก
    // ถ้าเรียกตรง ๆ บน null สคริปต์จะพังตรงนี้ ไปไม่ถึงโค้ดโหลดชุดคำถามข้างล่าง หน้าแก้ไขเลยว่างเปล่าทั้งที่ข้อมูลยังอยู่
    ...
    if (!currentPin) {
        // ชุดใหม่ที่ยังไม่เคยบันทึก: มีร่างค้างในเครื่องก็เปิดต่อได้เลย
        ...
        saveLocked = true;
        render();
        loadFromServer(currentPin).then((status) => {
            const draft = readDraft();
            if (status === 'ok') {
                saveLocked = false;
                // ข้อมูลบนเซิร์ฟเวอร์คือของจริง ร่างในเครื่องที่ต่างออกไปห้ามเอามาทับเงียบ ๆ ต้องถามก่อน
                // (ของเดิมโหลดร่างทับทุกครั้ง ร่างที่ค้างจากตอนบันทึกไม่สำเร็จเลยทำให้ข้อมูลที่แก้ไว้ "หาย")
                if (draft && snapshot(draft.title, draft.slides) !== snapshot(titleInput.value, slides)
                    && confirm(KG.i18n.t('make.useDraft'))) {
                    applyDraft(draft);
                } else {
                    localStorage.removeItem(STORAGE_KEY);
                }
            } else if (status === 'missing') {
                // ไม่มีชุดนี้แล้วหรือไม่ใช่ของเรา: เริ่มเป็นชุดใหม่ ไม่งั้นกดบันทึกจะโดนปฏิเสธตลอด
                saveLocked = false;
                currentPin = null;
                history.replaceState(null, '', location.pathname);
                alert(KG.i18n.t('make.loadFailed'));
            } else if (draft) {
                // เซิร์ฟเวอร์ติดต่อไม่ได้แต่มีร่างของชุดนี้: เปิดร่างให้ทำต่อ กดบันทึกได้เมื่อเซิร์ฟเวอร์กลับมา
                // PIN ยังเก็บไว้ บันทึกแล้วจะทับชุดเดิม ไม่กลายเป็นชุดใหม่ซ้ำ
                saveLocked = false;
                applyDraft(draft);
                alert(KG.i18n.t('make.loadErrorDraft'));
            } else {
                // ติดต่อไม่ได้และไม่มีร่าง: ล็อกปุ่มบันทึกไว้ (หน้าเปล่าห้ามไปทับของจริง) ให้รีเฟรชใหม่
                alert(KG.i18n.t('make.loadError'));
            }
            render();
        });
    }
