document.addEventListener('DOMContentLoaded', () => {
  ...
  const el = {
    ...
    waitPin: document.querySelector('#wait-pin'),
  ...
  const MODE_LOOK = {
    ...
    blocks:    { icon: '🧱', color: '#0d8020' },
    boss:      { icon: '🐉', color: '#b91c1c' },
    tower:     { icon: '🏗️', color: '#0f766e' },
  ...
  const renderResult = (msg, result, justEliminated) => {
    ...
    // ฉากบอสอยู่บนจอใหญ่ มือถือบอกเลือดบอสที่เหลือพอ
    if (msg.mode === 'boss' && msg.boss) {
      message += ' · ' + t('game.bossHp', { hp: msg.boss.hp, max: msg.boss.maxHp });
    }
  ...
  const renderFinal = (msg) => {
    ...
    } else if (msg.mode === 'boss' && msg.boss?.result) {
        el.question.textContent = t(msg.boss.result === 'win' ? 'game.bossWin' : 'game.bossLose')
            + ' ' + t('game.finalRank', { medal, place, total: msg.players.length });
    } else {
  ...
  // นั่งรอ host กดเริ่มอยู่บนหน้านี้เลย เมื่อก่อนรออยู่หน้า lobby แล้วค่อยย้ายมาตอนคำถามมาถึง
  // ซึ่งต้องโหลดหน้าใหม่ทั้งหน้า + ต่อ WebSocket ใหม่ ทั้งที่นาฬิกาข้อนั้นเดินไปแล้ว
  let lastPlayers = [];
  let preloadDraw = null;

  const paintWaiting = () => {
    el.playerCount.textContent = t('common.players', { n: lastPlayers.length });
    el.preload.textContent = preloadDraw ? preloadDraw() : '';
    ...
    lastPlayers.forEach((player) => {
      ...
      li.append(KG.avatar.el(player.avatar, 24), label);
      el.players.appendChild(li);
    });
  };

  const showWaiting = (players) => {
    lastPlayers = players ?? lastPlayers;
    el.waitPin.textContent = pin;
    el.waitView.hidden = false;
    // ซ่อนของฝั่งเล่นไว้ก่อน ไม่งั้นจะเห็นนาฬิกา "--" กับ "รอคำถาม..." ซ้อนกับข้อความรอของจริง
    el.timer.hidden = true;
    el.question.hidden = true;
    el.progress.hidden = true;
    el.home.hidden = false;
    redrawText = paintWaiting;
    paintWaiting();
    // เพลงรอเคยเล่นอยู่หน้า lobby ย้ายมาเล่นตรงนี้แทน (ดังจริงเมื่อผู้ใช้แตะจอปลดล็อกเสียงแล้ว)
    KG.sound.startMusic();
  };

  // กลับมาอยู่ล็อบบี้ (เช่น host เปิดห้องที่เล่นจบแล้วอีกครั้ง) ล้างของรอบเก่าบนจอให้หมด
  // ไม่งั้นพอรอบใหม่เริ่ม จะเห็นคะแนน/streak/ป้ายโหมด/สถานะตกรอบของรอบก่อนค้างอยู่
  const resetRound = () => {
    stopTimer();
    score = 0;
    streak = 0;
    ...
    el.question.dataset.i18n = 'game.waitingQuestion';
    el.question.textContent = t('game.waitingQuestion');
  };

  const hideWaiting = () => {
    el.waitView.hidden = true;
  ...
  const connect = () => {
    ...
    socket.onmessage = (event) => {
      ...
      if (msg.type === 'ERROR') {
        // ห้องไม่รู้จัก playerId นี้แล้ว: เลิกต่อใหม่ ล้าง id เก่า แล้วกลับไปให้ผู้เล่นกดเข้าห้องเอง
        // ถ้าปล่อยวนต่อใหม่ แท็บที่เปิดทิ้งไว้จะกลายเป็นผู้เล่นผีทุกครั้งที่มีรอบใหม่
        if (msg.code === 'sessionExpired') {
          sessionStorage.removeItem(`mangosgo.playerId.${pin}`);
          socket.onclose = null;
          socket.close();
      ...
      if (msg.type === 'JOINED') {
        ...
        paintStreak();
        // ใช้เวลาที่นั่งรอ host โหลดรูปคำถามเก็บเข้าแคชให้หมดก่อน พอถึงเวลาเล่นรูปจะขึ้นทันที
        KG.preload(msg.media, (done, total) => {
          preloadDraw = total
            ? () => (done < total ? t('lobby.preloading', { done, total }) : t('lobby.preloadDone', { total }))
            : null;
          el.preload.textContent = preloadDraw ? preloadDraw() : '';
        });
        return;
      ...
      // ยังไม่เริ่มเกม ก็นั่งรออยู่หน้านี้ ไม่ต้องเด้งไปหน้าอื่นแล้ว
      if (msg.type === 'ROOM_STATE' && msg.phase === 'lobby') { resetRound(); showWaiting(msg.players); return; }

      if (msg.type === 'QUESTION') { hideWaiting(); renderQuestion(msg); return; }
      ...
      if (msg.type === 'LEADERBOARD') {
        ...
        // ข้อที่มีคำตอบถูกหลายข้อ ไฮไลต์ทุกข้อที่ถูก
        const correctIndexes = msg.correctIndexes ?? [msg.correctIndex];
        el.answers.querySelectorAll('button').forEach((button) => {
          const isCorrect = correctIndexes.includes(Number(button.dataset.index));
          button.disabled = true;
          button.classList.remove('is-waiting');
          button.classList.toggle('is-correct', isCorrect);
          button.classList.toggle('is-dimmed', !isCorrect);
        });
  ...
  // ตัวเลือกที่มีคีย์คำแปล (ข้อถูก/ผิด) แสดงตามภาษาของเครื่องนี้ ตัวเลือกทั่วไปเป็นข้อความที่ครูพิมพ์
  function optionLabel(option) {
    return option.labelKey ? t(option.labelKey) : option.text;
  }
  ...
  KG.i18n.onChange(() => {
    ...
    paintStreak();
    if (lastQuestion) {
      el.progress.textContent = t('game.progress', { pin, index: lastQuestion.index + 1, total: lastQuestion.total });
      el.answers.querySelectorAll('button').forEach((button) => {
        const option = lastQuestion.options.find((item) => item.index === Number(button.dataset.index));
        if (option) button.textContent = optionLabel(option);
      });
    }
  });
