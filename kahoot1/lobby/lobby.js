document.addEventListener('DOMContentLoaded', async () => {
    // ต้องตรงกับ MAX_NICKNAME_LENGTH ใน server.js และ maxlength ของช่องกรอกใน lobby.html
    const NAME_LIMIT = 10;
    ...
    // ตัดชื่อที่จำไว้ให้พอดีขีดจำกัดด้วย เครื่องที่เคยเล่นตอนยังจำกัด 40 ตัวจะได้ไม่โชว์ชื่อยาว
    // แล้วค่อยโดนเซิร์ฟเวอร์ตัดทีหลังจนกลายเป็นคนละชื่อกับที่เห็นบนจอ
    el.nickname.value = (localStorage.getItem('mangosgo.player') || KG.nickname.random()).slice(0, NAME_LIMIT);
    ...
    const connect = (payload) => {
        ...
        socket.onmessage = (event) => {
            ...
            if (msg.type === 'ERROR') {
                ...
                if (!joined) {
                    // id ที่จำไว้ใช้ไม่ได้แล้ว ล้างทิ้ง รีเฟรชหน้าจะได้ไม่ต่อกลับด้วย id เดิมซ้ำอีก
                    if (msg.code === 'sessionExpired') sessionStorage.removeItem(idKey);
    ...
    el.enter.onclick = () => {
        const name = el.nickname.value.trim().slice(0, NAME_LIMIT);
