...
has 7 game modes, and runs in both Thai and English.
...
- **Only the web page folders are served** (`auth` `dashboard` `page` `Make` `host` `game` `lobby` `shared` `uploads`).
  The database, server source, `.git` and files in the project root cannot be fetched. The check runs on the resolved
  path, so `..` or `\` tricks do not get around it
- Malformed URL encoding or cookies cannot crash the server (bad URL → 400, bad cookie → treated as signed out), and any
  error escaping a request handler is caught at the top level and answered with 500
- Files under `/uploads/` are served with `Content-Security-Policy: sandbox` and `nosniff`, so an SVG with a script
  inside cannot run even when opened directly
- Anything users type (nicknames, questions, quiz titles) is put on the page with `textContent` only, never `innerHTML`

> Not built yet: password reset, email verification, and sign-in rate limiting. (Admins can change other people's roles
> from the dashboard.)
...
| **teacher** | Their own question sets (host / edit / delete), games already run, and a result report per game |
...
| Who | Opens | Does what |
| --- | --- | --- |
| **Teacher / admin** | `/Make/` | Write questions, add images, mark the correct answer, set time/points/type, hit Save to get a PIN — reopen and edit from any machine |
...
1. Build a question set at `/Make/` and hit **Save** — you get a 6-digit PIN (edit it later with ✏️ Edit on the dashboard)
...
├── modes.js         Rules for all 7 modes + the streak bonus formula
...
├── boss.js          The boss scene for Boss Raid
├── tower3d.js       The 3D scene for Tower Stack (ES module on three.js, loaded only when that mode is played)
├── vendor/          three.js r186 (three.module.js + three.core.js), kept locally so no internet is needed
├── mode-demo.js     Mode preview popup on the host's mode cards (plays the real scene with fake players)
...
- **A `playerId` the room no longer knows cannot reconnect** (gone longer than 15 seconds, server restarted, or the
  host started a new round). The server answers `sessionExpired` instead of quietly creating a new player, and the game
  page sends the player back to the lobby with their name filled in to join again — so a game tab left open from an
  old round never turns into a ghost player in the new lobby
...
Question sets are **stored on the server and tied to the account that made them**, so you can sign in
on any machine and keep editing. Hit **✏️ Edit** on the dashboard to open `/Make/?pin=PIN` with that set loaded.

- The first Save gives you a PIN; every later Save **writes over the same set**. The PIN stays the same,
  the dashboard never shows duplicates, and past results are kept
- A room for that set waiting in the lobby picks up the edits immediately; a room mid-question is left alone
- Only the owner can edit a set; admins can edit any set (and it still belongs to the original teacher)
- If saving to the server fails (e.g. the connection drops or the server is down) the work is kept as a local draft so nothing is lost
- Opening a set that has a local draft **different from the server copy asks first** which one to use — a draft never
  silently replaces the real data
- Opening a set while the server is unreachable: if there is a draft for that set it opens the draft; if not, **Save is
  locked** so an empty page cannot overwrite the real set. The PIN stays in the URL — just refresh once the server is back
- The **Home** button at the top right goes back to `/page/` (it does not warn about unsaved changes yet)

### Question settings (right-hand panel)

| Setting | Effect in the game |
| --- | --- |
| Question type: Quiz | 2–4 answers; empty slots do not appear in the game |
| Question type: True/False | Locked to two answers, "True/False". **Each player sees them in their own device's language**, whatever language the teacher saved in |
| Time limit | 10 / 20 / 30 / 60 s. "Apply to all questions" copies it to the whole set (Rush mode still forces 5 s) |
| Points | Standard ×1 · Double ×2 · No points ×0 — applied to both base points and bonus; no-points questions do not touch the streak |
| Answer options: Multiple correct answers | Click cards to add/remove correct answers (at least one always stays). Players still tap one answer and score if it is any correct one. In Block Jump every correct block survives |

**Question images** — upload, or drag a file onto the image box. A **✕ Remove image** button sits in the corner.

### Sound on the host screen

The lobby plays a relaxed loop while players join. Starting the game switches to a faster track with drums that
runs through questions and reveals, then stops for the podium. Music sits below the sound effects so the
countdown ticks stay audible, and the 🔊 button mutes everything. Browsers block audio until the page is
clicked, so click the host page once before the music can start.

---

## Game modes

Clicking the **Black Hole Run / Block Jump / Boss Raid / Tower Stack** cards on the host screen opens a popup that loops a
preview of that mode (the real scene renderer with 4 fake players) plus short captions that light up with the
action, so people get it without reading. Close it with "Got it", ✕, Esc or by clicking outside; the mode stays selected.
...
| 🐉 **Boss Raid** | The whole room fights one boss. Correct answers deal damage; if fewer than half get it right the boss hits back. Everyone wins or loses together |
| 🏗️ **Tower Stack** | A 3D scene: correct answers stack a block on your own tower, faster answers make taller blocks, wrong answers fall off. Tallest tower = top score |
...
The only mode where the room **cooperates** instead of competing. The scene lives on the **host screen only**;
phones keep the normal answer buttons and show how much health the boss has left.

- **Boss health** = players at Start × number of questions × 0.8 (if everyone answers every question
  correctly, even slowly, the room wins with a little to spare)
- **Hitting the boss:** a correct answer deals 1 damage, a fast one (first quarter of the time) deals 2.
  At the reveal, everyone who got it right jumps up and hits the boss and the health bar drops
- **The boss hits back** when fewer than half the room gets a question right — the team loses 1 of 3 hearts
- **Game over:** boss health reaches 0 = the room wins immediately · hearts run out, or the questions run
  out with the boss still alive = the room loses

Individual scores still use Classic scoring, so the podium still appears. Tune the difficulty with
`BOSS_HP_RATIO` / `BOSS_HEARTS` in `server/config.js`.

### How Tower Stack works

The only **3D** mode (three.js). The scene lives on the **host screen only**; phones never load three.js and keep
the normal answer buttons.

- **Tower height = total score, exactly** (1000 points = 1 unit tall). Scoring is plain Classic, so the towers always
  match the final podium
- **Correct:** one block, as tall as the points earned on that question (fast answers score more, so the block is taller) ·
  **wrong:** a block drops, clips the edge and falls off · **no answer:** no block
- The tallest tower wears a floating **crown**. Towers are laid out by name, not rank, so nobody's tower jumps around
- The server records per-question points in `player.gains` and attaches them to the player list in this mode only,
  so the host can rebuild every tower from scratch — refreshing mid-game loses nothing
- With more than 16 players the towers form several rows and only the top 5 get name labels, otherwise they overlap
- `shared/tower3d.js` is loaded with `import()` the first time the mode is played, so rooms playing other modes never
  download three.js (~2 MB)
- **No WebGL** (old projector PCs, hardware acceleration off): the host falls back to the normal question screen and the
  game carries on
...
The dictionary lives in exactly one place — [`shared/i18n.js`](shared/i18n.js) — with 260
...
being sent. `ERROR` carries a `code` for the page to translate (e.g. `pinNotFound` `nameTaken` `roomFull`
`sessionExpired` `roomClosed`), and mode names are stored as
...
| Add a new page folder | Add the folder name to `PUBLIC_DIRS` in `server/api.js`, otherwise the server answers 404 (files in the project root, such as `test.html`, are not served either) |
...
| Change what question settings do (points / true-false / multiple correct) | Converted in `normalizeSlide()` in `server/rooms.js` · scored in the `ANSWER` handler in `server/socket.js` · settings UI in `Make/make.js` |
| Change the sounds | `shared/sound.js` — synthesised with Web Audio, there are no files to swap. The lobby and in-game music live in `TRACKS` |
...
| `is-boss` | Boss Raid mode | Question hidden, only the boss scene |
| `is-tower` | Tower Stack mode | Question hidden, only the 3D tower scene (removed again if the scene fails to load) |
...
mechanic) · `blocks` (answer-platform stage) · `boss` (Boss Raid) · `tower` (attach per-question points for Tower Stack) · `endWhenAlive` (how many survivors ends the game)
...
3. **Reopening a finished room starts a new round straight away.** Hitting "▶ Host a game" (or opening
   `/host/?pin=` again) puts the room back in an empty lobby. **Everyone has to join again every round:** last round's
   players are all removed, and anyone whose game page is still open is sent back to the lobby (name filled in) to rejoin.
   Past results are kept in the dashboard report, but note
   that refreshing the host page while the podium is still showing also starts a new round
4. **No HTTPS/wss**, which modern browsers will block on a real domain
5. **Teachers can create unlimited question sets** — there is no per-account quota (you must be signed in as a teacher or admin)
...
- The database currently holds 186 question sets, most of them created during testing. Safe
  to delete
...
| `POST /api/quizzes/delete` | Delete a question set (owner or admin). Any open room for that set is closed immediately and every screen gets `ERROR` code `roomClosed` |
| `POST /api/users/role` | Change someone's role (admin only) |
| `POST /api/quizzes` | The Make page saves a question set and gets a PIN back (teacher/admin only) — include `pin` to save over an existing set (owner or admin) |
| `GET /api/quizzes/get?pin=` | Reopen a set for editing at `/Make/?pin=` from any machine (owner or admin) |
